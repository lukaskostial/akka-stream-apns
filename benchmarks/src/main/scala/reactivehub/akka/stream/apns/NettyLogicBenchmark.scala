...
class NettyLogicBenchmark {
  ...
  def setup(): Unit = {
    ...
    server = new EchoServer(serverGroup)
  }
  ...
  def testNettyBridge(blackHole: Blackhole): Unit = {
    val f = source
      .via(new NettyClient(server.address, clientGroup, data.length))
      .runWith(Sink.foreach(blackHole.consume))(materializer)
    Await.result(f, 30.seconds)
  }
  ...
  def testNettyClient(blackHole: Blackhole): Unit = {
    val channel = clientChannel(server.address, clientGroup,
    ...
    for (i ← 1 to operations) lastWrite = channel.writeAndFlush(data)

    lastWrite.addListener(new ChannelFutureListener {
